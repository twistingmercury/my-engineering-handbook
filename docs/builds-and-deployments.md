...
Here is an example of a multi-stage build from the [docker-ci/builds/api/build](https://github.com/twistingmercury/docker-ci-builds/tree/develop/api/build) project. The Dockerfile handles the compilation within controlled build stages, while the `build.sh` orchestrator script coordinates the overall process including installing [linters and security scanners](https://github.com/twistingmercury/docker-ci-builds/blob/41c3034cc6b0ac6714db5bc38f32313dca389faa/api/build/Dockerfile#L9), building the Docker [image](https://github.com/twistingmercury/docker-ci-builds/blob/1d99ff0962563bc7adf0ce51cb6a9779b8c95619/api/build/build.sh#L14C1-L28C2), running the end-to-end [tests](https://github.com/twistingmercury/docker-ci-builds/blob/1d99ff0962563bc7adf0ce51cb6a9779b8c95619/api/build/build.sh#L30), and preparing artifacts for the CI pipeline.
...
- [docker-compose.yaml](https://github.com/twistingmercury/docker-ci-builds/blob/develop/api/tests/docker-compose.yaml): This sets up the infrastructure the tests will needs. In the case of example API, we need to set up the following:
  - `otel_collector`: This is so that traces from the example API will have a working endpoint, and not create any unnecessary errors.
  - `project_databases`: This is a custom image that we use that contain the database accessed by example API.
